...
app.get(
  ...
  async (c) => {
    ...
    const reqWidth = c.req.valid("query").width;
    const reqHeight = c.req.valid("query").height;
    const origFileGlob = [
      path.resolve(IMAGES_PATH, `${reqImgId}`),
      path.resolve(IMAGES_PATH, `${reqImgId}.*`),
      path.resolve(IMAGES_PATH, `${reqImgId}-${reqWidth}-${reqHeight}.*`),
    ];
    ...
    if (
      resImgFormat === origImgFormat && ((reqWidth == null &&
        reqHeight == null) ||
        origFilePath.includes(`${reqWidth}-${reqHeight}`))
    ) {
      // 画像変換せずにそのまま返す
    ...
    const resultFilename = reqWidth != null && reqHeight != null
      ? `${reqImgId}-${reqWidth}-${reqHeight}.${resImgFormat}`
      : `${reqImgId}.${resImgFormat}`;
    await fs.writeFile(path.resolve(IMAGES_PATH, resultFilename), resBinary);
